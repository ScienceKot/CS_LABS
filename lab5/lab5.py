...
def check():
    ...
    for i in range(len(fail)):
        item2 = fail[i]
        array2.append(' FAILED POLICY Description' + item2[0]['description'])
        global arr2copy
        arr2copy = array2
    ...
    frame2 = Frame(main_app, bd=10, bg='#000000', highlightthickness=5)
    ...
    changeBtn= Button(frame2, text='Change', command=change_failures, bg="white", fg="black", font=app_font, padx='10px', pady='3px')
    changeBtn.place(relx=0.745, rely=0.93)

    backupBtn = Button(frame2, text='Restore', command=restore, bg="white", fg="black", font=app_font, padx='10px', pady='3px')
    backupBtn.place(relx=0.835, rely=0.93)

    def exit():
        frame2.destroy()

    exit_btn = Button(frame2, text='Back', command=exit, bg="white", fg="black", font=app_font, padx='10px',
                      pady='3px')
...
def on_select_failed(evt):
    ...
    array2=localarr2
    array2=[x for x in arr2copy if x not in array2]
...
def import_audit():
    ...
    for struct in structure:
        if 'description' in struct:
            array.append(struct['description'])
        else:
            array.append('Error in selecting')
    vars.set(array)
...
lstbox = Listbox(frame, bg = "#000000", font = app_font, fg = "white", listvariable = vars, selectmode = MULTIPLE, width = 150, selectbackground = 'red', height = 34, highlightthickness = 3)
...
save_btn = Button(frame, bg="white", fg="black", font=btn_font, text="Save", width=13, height=1, command=save_config).place(relx=0.01, rely=0.01)
import_btn = Button(frame, bg="white", fg="black", font=btn_font, text="Import", width=13, height=1,command=import_audit).place(relx=0.01, rely=0.065)
download_btn = Button(frame, bg="white", fg="black", font=btn_font, text="Download", width=13, height=1,command=extract_file).place(relx=0.01, rely=0.12)
select_all_btn = Button(frame, bg="white", fg="black", font=btn_font, text="Select All", width=13, height=1,command=select_all).place(relx=0.01, rely=0.175)
deselect_all_btn = Button(frame, bg="white", fg="black", font=btn_font, text="Deselect All", width=13, height=1,command=deselect_all).place(relx=0.01, rely=0.229)
exit_btn = Button(frame, bg="white", fg="black", font=btn_font, text="Exit", width=13, height=1, command=main_app.quit).place(relx=0.01, rely=0.284)
global e
e = Entry(frame, bg="white", font=btn_font, width=25, textvariable=querry).place(relx=0.2, rely=0.04)
find_btn = Button(frame, bg="white", fg="black", font=btn_font, text="Find", width=8, height=1,command=find).place(relx=0.41, rely=0.035)
check_btn = Button(frame, bg="white", fg="black", font=btn_font, text="Check", width=8, height=1,command=check).place(relx=0.485, rely=0.035)




main_app.bind('<Return>', input_find)
main_app.mainloop()
